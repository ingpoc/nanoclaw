...
if printf '%s\n' "$changed" | rg -q '^(CLAUDE\.md|AGENTS\.md|docs/workflow/|docs/operations/|\.codex/config\.toml|\.claude/settings\.local\.json)'; then
  workflow_out=""
  mirror_out=""
  workflow_rc=0
  mirror_rc=0
  ...
  if [ "$workflow_rc" -ne 0 ] || [ "$mirror_rc" -ne 0 ]; then
    echo "posttool warning: workflow/mirror checks are failing after edits" >&2
