...
describe('dispatch payload validation', () => {
  ...
  it('rejects missing request_id', () => {
    ...
    expect(valid).toBe(false);
    expect(errors).toContain('request_id is required for worker dispatch');
  });
...
describe('request_id linkage regression', () => {
  beforeEach(() => {
    _initTestDatabase();
  });

  it('dispatch without request_id is rejected by validator', () => {
    const payload = {
      run_id: 'task-link-reg-1',
      task_type: 'implement' as const,
      context_intent: 'fresh' as const,
      input: 'do work',
      repo: 'openclaw-gurusharan/nanoclaw',
      branch: 'jarvis-link-reg',
    ...
    const { valid, errors } = validateDispatchPayload(
      payload as unknown as import('./dispatch-validator.js').DispatchPayload,
    );
    expect(valid).toBe(false);
    expect(errors).toContain('request_id is required for worker dispatch');
  });

  it('dispatch with request_id links request to worker run', () => {
    createAndyRequestIfAbsent({
      request_id: 'req-link-reg-1',
      chat_jid: 'chat-reg',
      source_group_folder: 'andy-developer',
      user_message_id: 'msg-link-reg-1',
      user_prompt: 'build X',
      intent: 'work_intake',
      state: 'queued_for_coordinator',
    });

    insertWorkerRun('run-link-reg-1', 'jarvis-worker-1', {
      request_id: 'req-link-reg-1',
    });
    linkAndyRequestToWorkerRun(
      'req-link-reg-1',
    ...
    const request = getAndyRequestById('req-link-reg-1');
    expect(request?.worker_run_id).toBe('run-link-reg-1');
    expect(request?.state).not.toBe('queued_for_coordinator');
  });
});
